<script lang="ts">
  ...
  let {
    ...
    selected: TreeNode;
    vmsNode: TreeNode<(VCloud.vApp | Comparison<VCloud.vApp>) & {}>;
  } = $props();

  const vappComparison = vmsNode.data as Comparison<VCloud.vApp>;
  const vapp = vmsNode.data as VCloud.vApp;
</script>
...
<div class="p-4 space-y-4 @container flex flex-col">
  ...
  >
    <SummaryCard
      ...
      {isComparison}
      sourceValue={vappComparison.vms.filter((vm) => vm.status !== "added")
        .length}
      targetValue={vappComparison.vms.filter((vm) => vm.status !== "removed")
        .length}
    />
  </div>
  ...
  <div class="flex flex-col gap-2 min-w-fit max-w-6xl">
    <Table
      ...
      pagination
      data={vmsNode.data.vms}
      onRowClick={(row) => {
        selected =
          vmsNode.children?.find(
            (c) =>
              c.label ==
              (isComparison
    ...
    {#snippet VmNameCell(row: Row<(typeof vmsNode.data.vms)[number]>)}
