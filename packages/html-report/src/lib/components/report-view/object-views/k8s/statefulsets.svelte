<script lang="ts">
  import type { DataCenter, StatefulSet } from "@envops-cms/model";
  import {
    valueFromComparison,
    type Comparison,
...
<div class="p-4 space-y-4 @container flex flex-col">
  ...
  <p class="text-xl font-semibold">Stateful Sets</p>
  ...
  <div class="flex flex-col gap-2 min-w-fit max-w-6xl">
    <Table
      ...
      pagination
      data={k8sNode.data.statefulSets}
      onRowClick={(row) => {
        let setsNode = k8sNode.children?.find(
          (c) => c.label == "Stateful Sets"
        );
    ...
    >
      {#snippet empty()}
        <p class="p-4 text-gray-500">No Stateful Sets</p>
      {/snippet}
    </Table>

    {#snippet StatefulSetNameCell(
      row: Row<StatefulSet | Comparison<StatefulSet>>
    )}
      {@const comparison = row.data as Comparison<StatefulSet>}
      {#if isComparison}
