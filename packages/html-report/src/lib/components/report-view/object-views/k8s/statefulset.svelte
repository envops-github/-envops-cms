<script lang="ts">
  import type { DataCenter, StatefulSet, Pod } from "@envops-cms/model";
  import {
    valueFromComparison,
    type Comparison,
  ...
  let {
    ...
    selected: TreeNode;
    statefulSetNode: TreeNode<(StatefulSet | Comparison<StatefulSet>) & {}>;
  } = $props();

  const statefulSetComparison = statefulSetNode.data as Comparison<StatefulSet>;
  const statefulSet = statefulSetNode.data as StatefulSet;
</script>
...
<div class="p-4 space-y-4 @container flex flex-col">
  ...
  >
    <SummaryCard
      ...
      baseValue={statefulSet.pods.length}
      {isComparison}
      sourceValue={statefulSetComparison.pods.filter(
        (n) => n.status !== "added"
      ).length}
      targetValue={statefulSetComparison.pods.filter(
        (n) => n.status !== "removed"
      ).length}
    />
  </div>
  <p class="text-xl font-semibold">Pods</p>
  ...
  <div class="flex flex-col gap-2 min-w-fit max-w-6xl">
    <Table
      ...
      pagination
      data={statefulSetNode.data.pods}
      onRowClick={(row) => {
        statefulSetNode.open = true;

        selected =
          statefulSetNode.children?.find(
            (c) =>
              c.label ==
              (isComparison
    ...
    >
      {#snippet empty()}
        <p class="p-4 text-gray-500">No Pods</p>
      {/snippet}
    </Table>

    {#snippet PodNameCell(row: Row<Pod | Comparison<Pod>>)}
      {@const comparison = row.data as Comparison<Pod>}
      {#if isComparison}
